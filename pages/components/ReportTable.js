...
export default function ReportTable({ cookieStands, onDeleteCookieStand }) {
  // Handle the case when cookieStands is empty or undefined
  if (!cookieStands || cookieStands.length === 0) {
    // Display message when there are no cookie stands
    return <h2 className="text-2xl font-bold mt-8">No Cookie Stands Available</h2>;
  }

  // Calculate the totals for each hour
  const totalData = Array.from({ length: 14 }, (_, index) => {
    const sum = cookieStands.reduce((acc, stand) => acc + (stand.hourly_sales[index] || 0), 0);
    return sum;
  });
